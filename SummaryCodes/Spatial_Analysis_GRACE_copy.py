...
#fig.tight_layout()
fig.suptitle(name, fontsize=14, y=0.95)
fig.supylabel(ylabel, fontsize = 11, x=0.08)
#ax[1,1].plot(ds['Reservation'], label='Reservation', color='#8d5a99')
ax[0,0].plot(ds['Regulated with CAP'], "+-", label='Regulated with CAP', color=c_2) 
...
# For the actual figure
# fig, ax = plt.subplots(1,2,figsize=(12,4)) # horizontal
# fig.supylabel(ylabel, fontsize = 12, x=0.08) 

fig, ax = plt.subplots(2,1,figsize=(8,9)) # Stacked
fig.supylabel(ylabel, fontsize = 12, x=0.04)

#fig.tight_layout()
# fig.suptitle(name, fontsize=14, y=0.95)

ax[0].plot(cm_df_year, color='black', label='Arizona Average', lw = linewidth)
ax[1].plot(ds['Upper Colorado River - Mixed'],'--', lw=linewidth,color=mixed, label='Upper Colorado - Mixed')
ax[1].plot(ds['Norh - Mixed'], lw=linewidth,color=mixed, label='North - Mixed')
# ax[1].plot(ds['Central - Mixed'], color=c_11, label='Central - Mixed')
# ax[1].plot(ds['Northwest - GW Dominated'], color=c_7, label='Northwest - GW Dominated')
ax[1].plot(ds['Northeast - GW Dominated'], '--',lw=linewidth,color=GWdom, label='Northeast - GW Dominated')
ax[1].plot(ds['South central - GW Dominated'], 'v-',lw=linewidth,color=GWdom, label='South central - GW Dominated')
ax[1].plot(ds['Southeast - GW Dominated'], lw=linewidth,color=GWdom, label='Southeast - GW Dominated')
ax[1].plot(ds['Regulated with CAP'], lw=linewidth,label='Receives CAP (Regulated)', color=cap) 
# ax[1].plot(ds['Regulated without CAP'], "+-", label='Regulated without CAP', color=c_3) 
ax[1].plot(ds['Lower Colorado River - SW Dominated'], lw=linewidth,color=swdom, label='SW Dominated')
...
# ax[0].axvspan(a, b, color=drought_color, alpha=0.5, lw=0, label="Drought")
# ax[0].axvspan(c, d, color=drought_color, alpha=0.5, lw=0)
# ax[0].axvspan(e, f, color=drought_color, alpha=0.5, lw=0)
# ax[0].axvspan(g, h, color=drought_color, alpha=0.5, lw=0)
# ax[0].axvspan(i, j, color=drought_color, alpha=0.5, lw=0)
# ax[0].axvspan(k, l, color=drought_color, alpha=0.5, lw=0)

# ax[1].axvspan(a, b, color=drought_color, alpha=0.5, lw=0)
# ax[1].axvspan(c, d, color=drought_color, alpha=0.5, lw=0)
# ax[1].axvspan(e, f, color=drought_color, alpha=0.5, lw=0)
# ax[1].axvspan(g, h, color=drought_color, alpha=0.5, lw=0)
# ax[1].axvspan(i, j, color=drought_color, alpha=0.5, lw=0)
# ax[1].axvspan(k, l, color=drought_color, alpha=0.5, lw=0)
...
ax[0].legend(loc = [0.01, 0.01], fontsize = fsize)
ax[1].legend(loc = [0.01, 0.01], fontsize = fsize)
# 
fig.set_dpi(600)
